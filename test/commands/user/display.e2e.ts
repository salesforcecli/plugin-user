...
describe('UserDisplay tests', () => {
  ...
  if (!process.env.LOCALONLY) {
    ...
    it('includes the password and alias', async () => {
      const passwordResult = await exec2JSON('sfdx force:user:password:generate --json', {
        cwd: testProjectName,
      });
      expect(passwordResult.status).equal(0);
      const commandResult = await exec2JSON('sfdx force:user:display --json', {
        cwd: testProjectName,
      });
      expect(meetsRequiredShape(commandResult)).to.equal(true);
      ...
      await exec(`sfdx alias:set ${testAlias}=${commandResult.result.username as string}`);
      ...
      expect(commandResultWithAlias.result).to.have.property('alias').equal(testAlias);
    });
